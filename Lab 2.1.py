...
root = Tk()
canvas = Canvas(root, width=1200, height=1200, bg="lightgrey")
...
class Pyramide:
    ...
    def MoveForward(self, a, m):
        b = np.array([[math.cos(m * 0.01), 0, -math.sin(m * 0.01)],
                      [0, 1, 0],
                      [math.sin(m * 0.01), 0, math.cos(m * 0.01)]])
        # b = np.array([[0.5, 0, 0],
        #               [0, 0.5, 0],
        #               [0, 0, 0]])
        a = b.dot(a)
    ...
    def Move(self, k, a):
        for i in range(1, k):
            ...
            # X += X
            # Y += Y
            canvas.create_text(800, 10, fill="darkblue", font="Times 20 italic bold", text="Зрізана піраміда , обертання по Y")
